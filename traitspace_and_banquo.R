...
banquo <- function(P_S_E, tr, intercept=1, mu=-1.5, sigma =0, intra = NULL, rho = 0, avg.out = TRUE, out_alpha = FALSE){
  ...
  if (length(mu) ==1 & length(sigma) == 1 & ncol(tr) == 1 & ncol(P_S_E) == nrow(tr)){
    alphas <- interaction.matrix(scale(tr[,1]), intercept = intercept, mu = mu, sigma = sigma, intra = intra)
  ...
  if (!avg.out){
    ...
    if (any(P_S_E_all_interactions < 0)){
      P_S_E_interactions_unnorm <- sweep(P_S_E_interactions_unnorm, 1, apply(P_S_E_interactions_unnorm, 1, min), "-")
    }
    P_S_E_interactions <- sweep(P_S_E_interactions_unnorm, 1, apply(P_S_E_interactions_unnorm, 1, sum), "/")
    
    if (out_alpha){
      out <- list(P_S_E_interactions, alphas)
    }else{
      out <- P_S_E_interactions
    }
    return(out)
  }
}
